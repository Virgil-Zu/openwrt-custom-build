...
if [ -n "${target}" ] && [ -n "${sub_target}" ] && [ -n "${device}" ]; then
	...
	# gcc version !!!
	mkdir -p toolchain
    wget "https://downloads.openwrt.org/releases/${1#v}/targets/${target}/${sub_target}/openwrt-sdk-${1#v}-${target}-${sub_target}_gcc-7.5.0_musl.Linux-x86_64.tar.xz" -q
	...
	md5_value=$(echo "$kernel" | sed -n '/^kernel/ s/.*\([0-9a-fA-F]\{32\}\).*/\1/p')
	if [ -z "$md5_value" ]; then
